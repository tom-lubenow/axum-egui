...
/// Server function using MessagePack encoding.
///
/// MessagePack is more compact than JSON (~30% smaller) and faster to serialize.
/// Use it for large payloads or performance-critical endpoints.
#[server(msgpack)]
pub async fn multiply_msgpack(a: i32, b: i32) -> Result<i32, ServerFnError> {
    println!("Server (msgpack): multiplying {} * {}", a, b);
    Ok(a * b)
}
